 ...
 * with that instance of TorAgent.
 *
 * @returns {Promise}
 */
TorAgent.create = co.wrap(function*() {
  var ports = yield openports(1);
  var res = yield tmp.dirAsync({template: '/tmp/toragent-XXXXXX'});
  var tor = yield Tor.spawn(ports[0], res[0]);
 ...
 * @returns {Promise}
 */
TorAgent.prototype.destroyWrapper = co.wrap(function*() {
  this.defaultDestroy();
  yield this.tor.destroy();
});
